...
public class SessionHandlerServlet extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
         ...
         */
        HttpSession session = req.getSession(); // JSESSIONID를 이용하여 session을 생성(존재하면 호출)
        System.out.println("session 객체의 default 만료 시간(초): " + session.getMaxInactiveInterval());

        session.setMaxInactiveInterval(60 * 10);    // session 만료 시간 10분으로 설정
        System.out.println("session 객체의 수정 후 만료 시간(초): " + session.getMaxInactiveInterval());

        System.out.println("session id(해당 session의 JSESSIONID): " + session.getId());
