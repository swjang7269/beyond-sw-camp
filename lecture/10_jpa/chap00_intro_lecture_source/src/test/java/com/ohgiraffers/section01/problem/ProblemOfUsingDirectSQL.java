...
public class ProblemOfUsingDirectSQL {
    ...
    /* 설명. JDBC API를 이용해 직접 SQL을 다룰 때 발생할 수 있는 문제점 -> 이러한 문제점 때문에 우리는 JPA를 쓰겠다.
     *  1. 데이터 변환, SQL문 작성, JDBC API 코드 등의 중복 작성(개발 시간 증가, 유지보수성 저하)
     *  2. SQL에 의존하여 개발(DB의 column등이 수정되면 java 코드가 수정되어야 한다. -> DB에 종속)
     *  3. 패러다임 불일치(상속, 연관관계, 객체 그래프 탐색, 방향성)
     *  4. 동일성 보장 문제
     *
     * 설명. BUT 복잡하지 않은 쿼리에 대해서는 매우 강력하다.
     */
    ...
    void testAssociationObject() throws SQLException {
        ...
        while (rset.next()) {
            ...
            // category 객체와 연결된 부분에서 문제가 발생(Association 조인 관계)
    ...
    /* 설명. 3-2. 연관관계 문제, 객체 그래프 탐색 문제, 방향성 문제 (JOIN과 관련된 문제: JOIN은 양방향이 가능하다.(무방향))
    ...
    void testEquals() throws SQLException {
        // given
        String query = "SELECT MENU_CODE, MENU_NAME FROM TBL_MENU WHERE MENU_CODE = 10";
