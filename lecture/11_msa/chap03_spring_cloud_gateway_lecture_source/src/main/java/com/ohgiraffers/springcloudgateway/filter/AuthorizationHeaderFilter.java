...
public class AuthorizationHeaderFilter extends AbstractGatewayFilterFactory<AuthorizationHeaderFilter.Config> {
    ...
    public GatewayFilter apply(Config config) {
        return (exchange, chain) -> {
            ...
            /* 설명. request header에 "Authorizatio"이라는 키 값이 없다면 토큰 없이 요청이 온 것으로 판단 */
            if(!request.getHeaders().containsKey(HttpHeaders.AUTHORIZATION)) {
                return onError(exchange, "No authorization heater", HttpStatus.UNAUTHORIZED);
            }

            /* 설명. 토큰을 들고 왔다면 추가 검증 */
            HttpHeaders headers = request.getHeaders(); // apache말고 spring으로
            ...
            /* 설명. "Authorization"이라큰 키 값으로 request header에 담긴 것 추출(사용자가 들고온 JWT 토큰) */
