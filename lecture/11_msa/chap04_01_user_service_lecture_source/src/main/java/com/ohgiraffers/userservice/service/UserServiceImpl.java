...
public class UserServiceImpl implements UserService {
    ...
    public void registUser(UserDTO userDTO) {
        /* 설명. 회원가입 할 때 고유 번호 할당 */
        userDTO.setUserId(UUID.randomUUID().toString());

        /* 설명. Entityfh modelMapper로 매핑 후 엔티티에 있는 encryptedPwd에 암호화된 값을 추가한다. */
        UserEntity registUser = modelMapper.map(userDTO, UserEntity.class);
        registUser.setEncryptedPwd(bCryptPasswordEncoder.encode(userDTO.getPwd())); // plane text -> difest
    ...
    public UserDetails loadUserByUsername(String email) throws UsernameNotFoundException {
        ...
        /* 설명. 사용자가 로그인 시 id를(email) 잘못 입력 했다면 */
        if(loginUser == null) {
            throw new UsernameNotFoundException(email + " 해당 회원은 존재하지 않습니다.");
        }

        /* 설명. DB에서 조회 된 해당 email의 회원 권한들을 가져와 List<GrantedAuthority>로 받음 */
    ...
    public UserDTO getUserById(String memNo) {
        ...
        // orderService에서 주문내역도 가져와보자(다른 도메인의 데이터를 가져와보자)
        ...
        userDTO.setOrders(orderList);   // 조회해온 정보 담기
        return userDTO;
    }
}
